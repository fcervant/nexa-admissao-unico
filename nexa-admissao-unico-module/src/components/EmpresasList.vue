<template>
    <div>
        ...
        <v-data-table hide-default-header :headers="headersCandidatos" :items="listCandidatos">
            <template v-slot:item="row">
              <v-card width="100%" class="mx-auto my-12" max-width="1400" variant="tonal" :loading="loading">              
                <div class="d-flex flex-no-wrap justify-space-between">
                  <div>
                    <v-avatar class="ma-3" size="x-large">
                    <td><img alt="img-candidato" v-bind:src="'data:image/jpeg;base64,' + row.item.foto " /></td>    
                    </v-avatar>
                    <!-- <v-card-title class="text-h6">Candidato(a)</v-card-title> -->
                  </div>
                  <div>
                  <v-card-title class="subtitle-1">{{row.item.nome}}</v-card-title>
                  <v-card-subtitle>
                    <div class="d-flex flex-no-wrap flex-column align-start justify-center">
                      <span class="body-2">{{row.item.cargo}}</span>
                      <div class="d-flex flex-no-wrap align-start justify-center">
                        <span style="padding-right:5px" class="caption font-weight-medium">Banco</span>
                        <span style="padding-right:5px" class="caption">{{row.item.banco}}</span>
                        <span style="padding-right:5px" class="caption font-weight-medium">Agência</span>
                        <span class="caption">{{row.item.agencia}}</span>
                      </div>
                      <v-divider vertical class="mx-4"></v-divider>
                      <div class="d-flex flex-no-wrap align-start justify-center">
                        <span class="overline font-weight-medium">Data de Admissão:</span>
                        <span class="overline">{{row.item.dataadm}} - </span>
                        <span class="overline font-weight-medium">Posição Unico:</span>
                        <span class="overline">{{row.item.position}}</span>                        
                      </div>
                    </div>
                  </v-card-subtitle>
                </div>
                <div>
                  <v-btn class="mx-2" fab dark small color="#FF5000" @click="onButtonClick(row.item.nome,row.item.position)">Contrata
...
"",
    ...
    created () {
      console.log("3 - EmpresasList.vue created...");
      const objEmpresas = this.loadItemsEmpresas();
      this.ajustaListaEmpresa(objEmpresas);
    },
    computed: {},
    mounted() {
      this.buscadadosEmpresa(),
      this.buscaDadosCandidatos("1001"),
      console.log("EmpresasList.vue mounted...")
    },
    methods: {
      async loadItemsEmpresas() {
        ...
        ajustaListaEmpresa(objEmpresas) {
          let array = objEmpresas;
          if (array.length > 0) {
      ...
      buscaDadosCandidatos(companyCode)     {
        ...
        console.log("CandidatosList Component mounted...");
